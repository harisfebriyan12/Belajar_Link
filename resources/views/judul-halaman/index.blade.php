    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-xl shadow-lg p-4 md:p-6 mb-8 header-section">
            ...
            <div class="search-container max-w-md">
                <form action="{{ route('judul-halaman.index') }}" method="GET">
                    <div class="relative">
                        ...
                        <input type="search" name="search" placeholder="Cari judul atau deskripsi..." value="{{ request('search') }}"
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="table-row bg-white hover:bg-gray-50">
                            <td data-label="No" class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                <span class="inline-flex items-center justify-center h-8 w-8 rounded-full bg-blue-100 text-blue-600 font-semibold">
                                    {{ $judulHalamans->firstItem() + $index }}
                                </span>
                            </td>
                            <td data-label="Judul" class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ Str::limit($judul->judul, 30) }}</td>
                            <td data-label="Deskripsi" class="px-6 py-4 text-sm text-gray-600">{{ Str::limit($judul->deskripsi, 60) }}</td>
                            <td data-label="Status" class="px-6 py-4 text-sm">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium transition-colors {{ $judul->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                            ...
                            <td data-label="Aksi" class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center justify-end space-x-3">
                                    <button class="action-btn p-2 text-gray-500 hover:text-blue-600 hover:bg-blue-100 rounded-full transition-colors detail-judul-btn"
                                            title="Detail {{ e($judul->judul) }}"
                                            data-modal-open="detail-judul-modal"
                                            data-judul="{{ e($judul->judul) }}"
                                            data-deskripsi="{{ e($judul->deskripsi) }}"
                                            data-avatar="{{ $judul->images ? asset('storage/' . $judul->images) : '' }}"
                                            data-status="{{ $judul->is_active ? 'Aktif' : 'Tidak Aktif' }}"
                                            data-status-class="{{ $judul->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}"
                                            data-created_at="{{ $judul->created_at->format('d M Y, H:i') }}"
                                            data-updated_at="{{ $judul->updated_at->format('d M Y, H:i') }}"
                                            aria-label="Lihat detail {{ e($judul->judul) }}">
                                    ...
                                    <button class="action-btn p-2 text-green-600 hover:text-green-800 hover:bg-green-100 rounded-full transition-colors duration-200 edit-judul-btn"
                                            title="Edit {{ $judul->judul }}"
                                            data-modal-open="edit-judul-modal"
                                            data-id="{{ $judul->id }}"
                                            data-judul="{{ e($judul->judul) }}"
                                            data-deskripsi="{{ e($judul->deskripsi) }}"
                                            data-avatar="{{ $judul->images ? asset('storage/' . $judul->images) : '' }}"
                                            data-is_active="{{ $judul->is_active ? 'true' : 'false' }}"
                                            aria-label="Edit {{ $judul->judul }}">
                                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                        </svg>
                                    </button>
                                    <form action="{{ route('judul-halaman.destroy', $judul->id) }}" method="POST" class="inline delete-form">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="action-btn p-2 text-red-600 hover:text-red-800 hover:bg-red-100 rounded-full transition-colors duration-200 delete-button"
                                                title="Hapus {{ e($judul->judul) }}"
                                                aria-label="Hapus {{ e($judul->judul) }}"
                                                onclick="confirmDelete('{{ $judul->id }}', '{{ e($judul->judul) }}')">
                                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-8 text-center text-gray-500 text-lg">
                                @if(request('search'))
                                    Tidak ada judul yang cocok dengan pencarian "{{ request('search') }}".
                                @else
            ...
            @if ($judulHalamans->hasPages())
                <div class="px-6 py-4 bg-gray-50 border-t pagination">
                    {{ $judulHalamans->links('vendor.pagination.tailwind') }}
                </div>
            @endif
        </div>
        ...
        <div id="tambah-judul-modal" class="fixed inset-0 bg-gray-900 bg-opacity-75 hidden items-center justify-center z-50 modal-bg" aria-hidden="true">
            <div class="bg-white rounded-2xl shadow-2xl p-6 sm:p-8 w-full max-w-md sm:max-w-lg modal-content transform animate__animated animate__fadeIn">
                ...
                <form action="{{ route('judul-halaman.store') }}" method="POST" class="space-y-5" enctype="multipart/form-data" id="tambah-form">
                    @csrf
                    <div>
                        <label for="tambah-judul" class="block text-sm font-medium text-gray-700">Judul</label>
                        <input type="text" name="judul" id="tambah-judul" value="{{ old('judul') }}"
                               class="mt-1 block w-full rounded-lg border-gray-200 shadow-sm focus:ring-2 focus:ring-blue-600 focus:border-blue-600 py-3 px-4 transition-all duration-200 @error('judul') border-red-500 @enderror"
                               required aria-describedby="judul-error">
                        @error('judul')
                            <p id="judul-error" class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <textarea name="deskripsi" id="tambah-deskripsi" rows="4"
                                  class="mt-1 block w-full rounded-lg border-gray-200 shadow-sm focus:ring-2 focus:ring-blue-600 focus:border-blue-600 py-3 px-4 transition-all duration-200 @error('deskripsi') border-red-500 @enderror"
                                  aria-describedby="deskripsi-error">{{ old('deskripsi') }}</textarea>
                        @error('deskripsi')
                            <p id="deskripsi-error" class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="file" name="images" id="tambah-images" accept="image/*"
                               class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 @error('images') border-red-500 @enderror"
                               aria-describedby="images-error">
                        @error('images')
                            <p id="images-error" class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
            ...
            <div class="bg-white rounded-2xl shadow-2xl p-6 sm:p-8 w-full max-w-md sm:max-w-lg modal-content transform animate__animated animate__fadeIn">
                ...
                <form id="edit-form" method="POST" class="space-y-5" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        @error('judul')
                            <p id="edit-judul-error" class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        @error('deskripsi')
                            <p id="edit-deskripsi-error" class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        @error('images')
                            <p id="edit-images-error" class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        <label class="inline-flex items-center">
